...
is
select e.c_id, c.c_name, e.c_id_no, c.c_unit, t.t_time, t.t_loc
from enroll e, course c, teach t
where e.s_id = sStudentId and(e.e_year-2000) = nYear
and (e.e_semester) = nSemester and
t.t_year - 2000 = nYear and t.t_semester = nSemester and
...
loop
fetch cur into cid, cname, cidno, cunit, ttime, tloc ;
exit when cur%notfound;

dbms_output.put_line('교시:' || to_char(ttime) ||'   과목번호:'||cid|| '   과목명:'|| cname|| '   분반:'||to_char(cidno)|| '   학점:' ||to_char(cunit) ||'   강의실:'||tloc);
